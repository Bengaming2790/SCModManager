...
# --- GitHub Downloader Window ---
GITHUB_USER = "Bengaming2790"
GITHUB_REPO = "SCModManager-RecPacks"
API_URL = f"https://api.github.com/repos/{GITHUB_USER}/{GITHUB_REPO}/contents/"
TARGET_DIR = os.path.join(os.getcwd(), "modpacks")
PROFILE_LIST = []
GITHUB_TOKEN = os.getenv("GITHUB_TOKEN")
HEADERS = {"Authorization": f"token {GITHUB_TOKEN}"} if GITHUB_TOKEN else {}

def open_modpack_window():
    def fetch_packs():
        resp = requests.get(API_URL, headers=HEADERS)
        resp.raise_for_status()
        return [i for i in resp.json() if i["type"] == "dir"]

    def download_pack(folder):
        ...
        prefix = f"{GITHUB_REPO}-main/{folder['name']}/"
        folder_files = [f for f in z.namelist() if f.startswith(prefix)]

        if not folder_files:
            messagebox.showwarning("Not found", f"No folder {folder['name']} in archive")
            return

        outdir = os.path.join(TARGET_DIR, folder['name'])
        ...
        for f in folder_files:
            ...
            if rel:
                os.rename(
                    ...
                    os.path.join(outdir, rel)
                )

        PROFILE_LIST.append(folder['name'])
        messagebox.showinfo("Success", f"Downloaded and created profile: {folder['name']}")
        refresh_profiles()
    ...
    def on_download():
        sel = pack_list.get(tk.ACTIVE)
        if sel:
            folder = next(f for f in packs if f["name"] == sel)
            download_pack(folder)

    modpack_window = tk.Toplevel(root)
    modpack_window.title("Mod Pack Downloader")
    modpack_window.geometry("400x450")

    try:
        packs = fetch_packs()
    except Exception as e:
        messagebox.showerror("GitHub Error", f"Failed to fetch modpacks:\n{e}")
        return

    tk.Label(modpack_window, text="Available Mod Packs:").pack()
    pack_list = tk.Listbox(modpack_window, height=8, exportselection=False)
    for f in packs:
        pack_list.insert(tk.END, f['name'])
    pack_list.pack(fill="x")

    btn = tk.Button(modpack_window, text="Download & Add Profile", command=on_download)
    btn.pack(pady=5)

    tk.Label(modpack_window, text="Installed Profiles:").pack()
    listbox = tk.Listbox(modpack_window, height=8)
    listbox.pack(fill="x")
